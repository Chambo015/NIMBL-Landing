<script setup lang="ts">
import { ref } from 'vue';

const targetEl = ref(null)
defineExpose({
    targetEl
})
</script>

<template>
    <div ref="targetEl" class="2xl:w-[126px] 2xl:h-[126px] w-[110px] h-[110px] aspect-square rounded-full absolute border border-[rgba(184,184,184,0.5)] bg-[rgb(0,0,0,0.15)] backdrop-blur-2xl cursor-pointer">
        <svg width="50" height="58" viewBox="0 0 50 58" class="w-[46%] h-[46%] absolute top-1/2 left-1/2 -translate-x-[40%] -translate-y-[50%]" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                d="M0.182074 0.908953L49.4279 29.341L0.182073 57.7731L0.182074 0.908953Z"
                fill="url(#paint0_linear_598_4502)" />
            <defs>
                <linearGradient
                    id="paint0_linear_598_4502"
                    x1="-23.4999"
                    y1="-4.99975"
                    x2="28.7794"
                    y2="-27.9323"
                    gradientUnits="userSpaceOnUse">
                    <stop stop-color="#9B1CBF" stop-opacity="0.77" />
                    <stop offset="0.508333" stop-color="#3672FE" />
                    <stop offset="1" stop-color="#03E7F3" />
                </linearGradient>
            </defs>
        </svg>
    </div>
</template>
